...
dataset <- na.omit(read.csv("~/Calinski_Harabasz_Thesis/Data/DatasetReali/neuroblastoma.csv"))
...
Quindi verifichiamo che applicando l'algoritmo di clustering K-means, la metrica di Calinski-Harabasz individua come miglior configurazione quella con il valore di k=5, distance="euclidean", nstart=100  rispetto a quelle scelte.
...
Quindi verifichiamo che applicando l'algoritmo di clustering DBSCAN, la metrica di Calinski-Harabasz individua come miglior configurazione quella con i valori di 𝜀=3 e minPts=11 rispetto a quelle scelte.
...
Per scegliere la configurazione migliore ci limitiamo ad analizzare il clustering per le configurazioni che vanno da 2 a 10 cluster cercando un ottimo locale con il valore di k minimo. Per l'algoritmo di clustering Hierarchical clustering, la metrica di Calinski-Harabasz individua come miglior configurazione quella con tipo complete-linkage rispetto alle altre opzioni con ottimo locale per k=4.
...
L'analisi effettuata tramite la metrica di Calinski-Harabasz indica come miglior risultato la combinazione di DBSCAN come algoritmo di clustering con configurazione di iperparametri eps=3.0 e minPts=11.
Notiamo inoltre come per K-means e DBSCAN il numero di cluster ottimali coincida(k=5) mentre per Hierarchical-clustering risulti diverso(k=4)
