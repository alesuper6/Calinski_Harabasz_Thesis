...
dataset <- na.omit(read.csv("~/Calinski_Harabasz_Thesis/Data/DatasetReali/Sepsis_SIRS.csv"))
...
Quindi verifichiamo che applicando l'algoritmo di clustering K-means, la metrica di Calinski-Harabasz individua come miglior configurazione quella con il valore di k=3, distance="euclidean" ed nstart=2 rispetto a quelle scelte.
...
list_eps <- c(18, 19.8, 21.6)

# Creazione lista con i valori di minPts
list_minPts <- c(8, 9, 10)
...
Quindi verifichiamo che applicando l'algoritmo di clustering DBSCAN, la metrica di Calinski-Harabasz individua come miglior configurazione quella con i valori di 𝜀=19.8, minPts=10 e k_out=2 rispetto a quelle scelte.
...
Per l'algoritmo di clustering Hierarchical clustering, la metrica di Calinski-Harabasz individua come miglior configurazione quella con tipo complete-linkage e distance=euclidean rispetto alle altre opzioni, con ottimo locale per k=2.
...
Notiamo inoltre come per DBSCAN e Hierarchical-clustering il numero di cluster ottimali coincida(k=2) mentre per K-means risulti diverso(k=3)